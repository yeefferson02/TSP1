<?php
	...
	if($loginCount != 0)
	{
		...
		$getAccountStatus = mysql_query("SELECT premium, premium_expiry from login where account_id = '$loginResult[0]'")or die(mysql_error());
		$getAccountStatus = mysql_fetch_array($getAccountStatus);
		
		if($getAccountStatus[0] == 1 && $getAccountStatus[1] < time())
		{
			$cancelPremium = mysql_query("UPDATE login set premium = 0, premium_expiry = 0 where account_id = '$loginResult[0]'");
		}
